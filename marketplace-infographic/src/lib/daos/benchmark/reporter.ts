...
function renderReport(results: BenchmarkResults): string {
  ...
  lines.push(
    `- LAW_003 violation rate (wide layout OFF → ON): ${formatRate(results.aggregate.law003ViolationRateBaseline)} → ${formatRate(results.aggregate.law003ViolationRateDaos)}`,
  );
  lines.push(
  ...
  lines.push(
    `- Average compositeProductAreaRatio (wide layout OFF → ON): ${results.aggregate.averageCompositeProductAreaRatioBaseline?.toFixed(2) ?? "n/a"} → ${results.aggregate.averageCompositeProductAreaRatioDaos?.toFixed(2) ?? "n/a"}`,
  );
  lines.push(
    `- compositePlacementFound rate (wide layout OFF → ON): ${formatRate(results.aggregate.compositePlacementFoundRateBaseline)} → ${formatRate(results.aggregate.compositePlacementFoundRateDaos)}`,
  );
  lines.push(
    `- extractAreaCorrected rate (wide layout OFF → ON): ${formatRate(results.aggregate.extractAreaCorrectedRateBaseline)} → ${formatRate(results.aggregate.extractAreaCorrectedRateDaos)}`,
  );
  lines.push(
  ...
  lines.push(
    `- LAW_003 soft resolved rate (wide layout OFF → ON): ${formatRate(results.aggregate.law003SoftResolvedRateBaseline)} → ${formatRate(results.aggregate.law003SoftResolvedRateDaos)}`,
    `- targetUnreachable rate (wide layout OFF → ON): ${formatRate(results.aggregate.targetUnreachableRateBaseline)} → ${formatRate(results.aggregate.targetUnreachableRateDaos)}`,
  );
  lines.push(
